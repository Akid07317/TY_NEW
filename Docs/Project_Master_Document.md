...
| 系统 | 第一版必须做 | 以后可扩展 |
|---|---|---|
| 移动 | `WASD` 地面移动，面向摄像机方向；走跑一体，不做体力条 | 冲刺、攀爬、特殊移动 |
...
| 闪避 | 定向闪避，成功穿招后累积 `AgilityGauge` | 完美闪避、空中闪避 |
...
|---|---|---|---|---|
| `PlayerBaseStatsSO` | 玩家基础属性 | HP、MP、Attack、Defense、速度 | 是 | 多角色模板 |
| `CombatBalanceSO` | 全局战斗参数 | 量表积累、格挡窗口、无敌帧、Hit Stop | 是 | 难度配置 |
